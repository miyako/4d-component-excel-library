...
$json_t:=XCEL_util_EXECUTE("window_get_rect")
...
If (Match regex:C1019("\\{\"left\":(.+?),\"top\":(.+?),\"width\":(.+?),\"height\":(.+?)\\}"; $json_t; 1; $match_position_al; $match_length_al))
	...
	XCEL_util_GET_PARAMETER($3; ->$left_r)
	XCEL_util_GET_PARAMETER($4; ->$top_r)
	XCEL_util_GET_PARAMETER($5; ->$width_r)
	XCEL_util_GET_PARAMETER($6; ->$height_r)
